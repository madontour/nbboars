...
else
{
  ...
  $description = "User Number: ". Dlookup("uid","mrbs_users","name='$user'");
  $description = $description . "\n" . "Mobile Num:  " . Dlookup("mobile","mrbs_users","name='$user'");
  $description = $description . "\n" . "Locality:  " . Dlookup("locality","mrbs_users","name='$user'");
  $description = $description . " ~ " . Dlookup("postcode","mrbs_users","name='$user'");
  //$description   = $default_description; original code
  $type          = $default_type;
  $room_id       = $room;
  $private       = $private_default;
...
//	echo "query string is ".$q;
	$rst = mysql_query($q);	
//	echo "recordset is ".$rst;
	if ($row = mysql_fetch_object($rst)) 
		return $row->$fld;
	else
		return NULL;
//     return "text from dlookup";
}
